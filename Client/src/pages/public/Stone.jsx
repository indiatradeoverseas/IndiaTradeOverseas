...
const TEASER_LISTINGS = [
  {
    id: "ST-BHU-01",
    region: "Gomtu / Phuentsholing Corridor",
    type: "Bhutan White 20MM",
    baseGrade: "Quartzite Crushed",
    package: "Dump Truck / Trailer",
    use: "RCC Roof & Commercial Works"
  },
  {
    id: "ST-PAK-02",
    region: "Pakur, Jharkhand Belt",
    type: "Pakur Black 30MM/40MM",
    baseGrade: "High-Density Basalt",
    package: "Loose Bulk Freight",
    use: "National Highway & Bridge Base"
  },
  {
    id: "ST-PAS-03",
    region: "Pasakha / Samtse Gate",
    type: "0MM Stone Dust & WMM",
    baseGrade: "Screened Fines",
    package: "Multi-Axle Truckloads",
    use: "Asphalt & Plastering Applications"
  },
  {
    id: "ST-PUG-04",
    region: "Pugli Loading Point",
    type: "Bhutan Black 60MM",
    baseGrade: "Mass Fill Grade",
    package: "Bulk Tipper Fleets",
    use: "Railway Ballast & Mass Civil"
  }
];
...
const PAKUR_RAW = [
  ...
  ['Sheikhpura',   [2380, 2380, 2230, 1780], [2450, 2450, 2350, 1900], [2500, 2500, 2350, 1900]],
  ['Madhubani',    [2605, 2605, 2455, 2005], [2675, 2675, 2525, 2125], [2725, 2725, 2125, 2125]],
...
export default function Stone() {
  ...
  const [showEntryGate, setShowEntryGate] = useState(() => {
    ...
    return !(
      localStorage.getItem('ito_stone_buyer_id') &&
  ...
  // ============================================================
  // MASTER DPR v4.0 — CANONICAL DIVISION VIEW
  // ============================================================
  useEffect(() => {
    pushDataLayerEvent('division_view', {
      division: 'STONE',
      page: '/stone'
    });
  }, []);
  ...
  useEffect(() => {
    const bgTimer = setInterval(() => {
      setHeroBgIndex(
        (prev) => (prev + 1) % HERO_CAROUSEL_IMAGES.length
      );
    }, 5000);
  ...
  // Persist the settled layer (storefront vs marketplace)
  useEffect(() => {
        ...
        try {
          ...
          if (res.success) {
            ...
            if (currentStatus === 'approved') {
              toast.success(
                "B2B Stone Profile Approved! Secure Terminal Activated."
              );
  ...
  // Buyer verification funnel events are handled inside BuyerEntryGate.jsx.
  const handleGateVerified = (
    ...
    // Existing event retained temporarily.
    // Canonical buyer verification events will be handled
    // centrally inside BuyerEntryGate.jsx.
    pushDataLayerEvent(
      'stone_distributor_verified',
      {
        division: 'STONE'
      }
    );
  ...
  // Only for destructive cases:
  // CRM record deleted (404) or explicitly rejected.
  const handleLogOut = () => {
    setUserAccessLayer(1);
    setDistributorId('');
    ...
    toast.success(
      "Secured terminal session locked."
    );
  };

  // Manual "exit" from the marketplace.
  ...
  // ============================================================
  // MASTER DPR v4.0 — PRODUCT INTENT
  // ============================================================
  const handleExploreProducts = () => {
    pushDataLayerEvent('product_intent', {
      division: 'STONE',
      intent: 'view_sourcing_options',
      source_page: '/stone'
    });
  ...
  const activeRateEntry = (
    rateDivision === 'PAKUR'
      ? PAKUR_RATES
      : BHUTAN_RATES
  ).find(
    (entry) =>
      entry.location === rateLocation
  );

  const activeRatePrice =
    rateDivision === 'PAKUR'
      ? activeRateEntry
          ?.rates?.[rateGrade]?.[
            PAYMENT_TERMS.find(
              (t) =>
                t.key === ratePaymentTerm
            ).priceField
          ] ?? null
      : activeRateEntry
          ?.rates?.[rateGrade] ?? null;
  ...
  const openRateQuoteDrawer = () => {
    ...
    const gradeLabel =
      rateDivision === 'PAKUR'
        ? PAKUR_SIZE_LABELS[rateGrade]
        : BHUTAN_TYPE_LABELS[rateGrade];
    ...
    const lotId = `${rateDivision}-${rateLocation
      .toUpperCase()
      .replace(/\s+/g, '-')}-${rateGrade.toUpperCase()}`;

    // Canonical quote_start fires when a valid
    // sourcing configuration actually opens the quote drawer.
    pushDataLayerEvent('quote_start', {
      division: 'STONE',
      source_page: '/stone',
      origin: rateDivision,
      delivery_location: rateLocation,
      grade: gradeLabel,
      payment_term:
        termLabel || undefined,
      lot_id: lotId
    ...
    setActiveDrawerLot({
      ...
      division:
        rateDivision === 'PAKUR'
          ? 'Pakur Stone'
          : 'Bhutan Stone',
      region: activeRateEntry.state
        ? `${rateLocation}, ${activeRateEntry.state}`
        : rateLocation,
      grade: termLabel
    ...
    <div className="bg-[#F4F2EE] text-[#37424B] antialiased min-h-screen font-serif selection:bg-[#C5A059]/30 selection:text-[#37424B]">
      ...
      {userAccessLayer === 4 && (
        <div className="min-h-[85vh] flex items-center justify-center py-20 px-4">
          <motion.div
            ...
            className="max-w-2xl bg-white rounded-2xl p-8 sm:p-12 shadow-2xl border border-[#DCCCB4] text-center space-y-6"
          >
            ...
            <h2 className="text-3xl font-serif text-[#37424B] uppercase tracking-wide">
              Buyer Account Under Review
            </h2>
            ...
            <div className="bg-[#F4F2EE] border border-[#DCCCB4] rounded-xl p-4 text-left text-xs font-sans text-[#6D6760] space-y-1.5 max-w-md mx-auto">
              <div className="font-bold text-[#37424B] uppercase tracking-wider text-[10px] font-mono">
                VERIFICATION CHECKLIST PIPELINE:
              </div>

              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-[#C5A059]" />
                Statutory Registration Audit (GSTIN / Udyam Matching)
              </div>

              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-[#A89E8E]" />
                BANT Method: Budget, Authority, Need, and Timeline Check
              </div>

              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-[#A89E8E]" />
                Non-Credit Protocol Confirmation (100% Advance Terms)
              </div>
            </div>
            ...
            <div className="pt-4">
              <button
                ...
                className="text-[10px] font-mono text-[#C5A059] hover:text-[#37424B] uppercase tracking-wider underline"
              >
            ...
            <div className="bg-[#37424B] text-white rounded-xl p-3 sm:px-6 sm:py-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border border-[#C5A059]/30 shadow-xl">
              ...
              <div className="flex items-center justify-between gap-2 w-full sm:w-auto">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-[9px] sm:text-[10px] font-mono tracking-widest text-[#C5A059] font-bold uppercase leading-none mb-0.5 truncate">
                      STONE DIVISION TERMINAL
                    </div>

                    <div className="text-xs sm:text-sm font-serif tracking-wider text-white uppercase font-medium truncate">
              ...
              <div className="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto">
                <div className="hidden md:flex flex-col text-right font-mono text-[10px] text-slate-300 border-r border-white/10 pr-4">
                  <span>
                    BHUTAN & PAKUR DESKS DISPATCH ACTIVE
                  </span>

                  <span className="text-[#C5A059]">
                    100% ADVANCE LEDGER SECURED
                  </span>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/70 backdrop-blur-xs">
                  ...
                  >
                    <div className="bg-[#37424B] text-white p-4 sm:p-6 flex justify-between items-center text-left shrink-0">
                      <div>
                        <div className="text-[9px] font-mono tracking-widest text-[#C5A059] font-bold uppercase">
                          Stone Sourcing Ledger
                        </div>

                        <h2 className="text-base sm:text-xl font-serif text-white uppercase tracking-wide">
                          My Active Stone Proposals
                        </h2>
                      </div>
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-300 hover:text-white bg-white/5 border border-white/10 rounded-lg"
                      >
                        <FiX size={18} />
                      </button>
                    </div>
                      ...
                      {myProposals.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {myProposals.map((prop) => (
                            <div
                              key={prop._id}
                              className="p-3.5 sm:p-4 rounded-xl border border-[#DCCCB4] bg-white space-y-3 text-xs"
                            >
                              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 border-b border-slate-100 pb-2.5">
                                <div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-bold font-mono text-[#37424B]">
                                      {prop.lotId}
                                    </span>
                                  ...
                                  {prop.status === 'approved' && (
                                    <button
                                      onClick={async () => {
                                        ...
                                        const loadingToast =
                                          toast.loading(
                                            `Preparing checkout for ${prop.lotId}...`
                                          );
                                              ...
                                              async function (
                                                ...
                                                try {
                                                  ...
                                                  if (
                                                    ...
                                                    await distributorApi.updateProposalStatus(
                                                      prop._id,
                                                      'paid'
                                                    );

                                                    toast.success(
                                                      `Payment verified for Lot ${prop.lotId}!`
                                                    );

                                                    // ========================================================
                                                    // MASTER DPR v4.0 — PAYMENT SUCCESS
                                                    // Fires only after successful verification + status update.
                                                    // ========================================================
                                                    pushDataLayerEvent(
                                                      'payment_success',
                                                      {
                                                  ...
                                                  console.error(
                                                    'Razorpay verify-payment failed:',
                                                    verifyErr
                                                      .response
                                                      ?.data ||
                                                      verifyErr
                              ...
                              <div className="grid grid-cols-2 gap-2 text-[11px] font-mono bg-[#F4F2EE] p-2.5 rounded-lg border border-[#DCCCB4]/40">
                                <div>
                                  ...
                                  <span className="font-bold text-slate-800">
                                    {prop.quantity?.toLocaleString()} MT
                                  </span>
                                </div>
                                ...
                                <div>
                                  ...
                                  <span className="font-bold text-[#37424B]">
                                    INR{' '}
                                    {(
                                      prop.estimatedValue ||
                                      prop.quantity *
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (
                            ...
                            return toast.error(
                              "Total exceeds Razorpay's single-transaction cap (₹5,00,000). Please pay invoices individually."
                            );
                          }
                          ...
                          const loadingToast =
                            toast.loading(
                              "Configuring transaction security manifest..."
                            );
                          ...
                          try {
                            ...
                            if (
                              ...
                              throw new Error(
                                orderResult?.message ||
                                  "Failed to create secure transaction token."
                              );
                            }
                              ...
                              handler:
                                async function (
                                  ...
                                  try {
                                    ...
                                    toast.success(
                                      "Transaction certified! Invoices cleared."
                                    );

                                    // ========================================================
                                    // MASTER DPR v4.0 — PAYMENT SUCCESS
                                    // Fires only after successful verification + proposal updates.
                                    // ========================================================
                                    pushDataLayerEvent(
                                      'payment_success',
                                      {
              ...
              <div className="bg-white border border-[#DCCCB4] rounded-xl p-4 sm:p-6 shadow-sm space-y-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-lg font-mono text-[10px] sm:text-[11px] font-bold uppercase tracking-wider border transition-all cursor-pointer ${
                      rateDivision === 'PAKUR'
                        ? 'bg-[#37424B] text-white border-[#37424B]'
                        : 'bg-[#F4F2EE] text-[#6D6760] border-[#DCCCB4]'
                    }`}
                  >
                    Pakur Stone
                  </button>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#F4F2EE] border border-[#DCCCB4] rounded-xl p-4">
                  ...
                  null ? (
                    <span className="text-xs font-sans text-slate-500 italic">
                      Not available at this location for the selected grade.
                    </span>
                  ) : (
                    <>
                      <div>
                        ...
                        <span className="text-xl font-mono font-extrabold text-[#37424B]">
                          INR{' '}
                          {activeRatePrice.toLocaleString()}
                ...
                {rateDivision ===
                  ...
                  <p className="text-[10px] text-slate-400 font-sans">
                    Rates per official Bhutan Stone rate card; up to ₹100/MT may be negotiable at final invoicing.
                  </p>
                )}
              </div>
            </div>
      ...
      {userAccessLayer <= 2 && (
        <>
          ...
          <div className="relative min-h-screen bg-[#37424B] overflow-hidden flex items-center pt-24 lg:pt-0">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-center py-12 lg:py-0">
              ...
              <div className="lg:col-span-5 space-y-3">
                {[
                  {
                    label: "01 • SOURCING ORIGIN",
                    title: "Bhutan & Pakur Mines",
                    desc: "Basalt and quartzite geological extraction."
                  },
                  {
                    label: "02 • NOMINAL SIZES",
                    title: "10mm, 20mm, 30mm, 40/60mm, Dust",
                    desc: "Mechanical crushing plant configurations."
                  },
                  {
                    label: "03 • LOGISTICS NETWORK",
                    title: "Jaigaon, Pasakha, Kishanganj Fleet",
                    desc: "Multi-axle tipper and dump truck routing."
                  },
                  {
                    label: "04 • TRADE LEDGER",
                    title: "COMMERCIAL RATE MATRIX",
                    desc: "Quotation and order confirmation process."
                  }
                ].map(
                  (card, idx) => (
                    <div
                    ...
                    >
                      ...
                      <div className="text-sm font-bold text-[#F4F2EE]">
                        {card.title}
                      </div>

                      <div className="text-[10px] text-slate-300 font-sans font-light">
                        {card.desc}
                      </div>
                    </div>
                  )
                ...
                (item, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center bg-white border border-[#DCCCB4] rounded-2xl p-6 sm:p-10 shadow-lg text-left ${
                        isEven
                          ? ''
                          : 'lg:flex-row-reverse'
                      }`}
                    >
                      ...
                      <div
                        className={`lg:col-span-6 space-y-4 font-sans ${
                          isEven
                            ? 'lg:order-2'
                            : 'lg:order-1'
                        }`}
                      >
                        <div className="space-y-1">
                          ...
                          <h3 className="text-2xl sm:text-3xl font-serif text-[#37424B] font-bold uppercase">
                            {item.title}
                          </h3>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#DCCCB4]/50">
                          {item.specs.map(
                            (
                              ...
                              <div
                                key={sIdx}
                                className="flex items-center gap-1.5 text-[11px] font-mono text-[#37424B]"
                              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                  [
                    "01",
                    "Share Requirement",
                    "Tell us the aggregate size, quantity and delivery location."
                  ],
                  [
                    "02",
                    "Source & Verify",
                    "We coordinate the relevant Bhutan or Pakur sourcing option."
                  ],
                  [
                    "03",
                    "Confirm Commercials",
                    "Availability, specification, pricing and delivery terms are confirmed."
                  ],
                  [
                    "04",
                    "Dispatch & Logistics",
                    "Dispatch and transport are coordinated through the agreed supply process."
                  ]
                ].map(
                  ([
                    number,
                    ...
                    <div
                      key={number}
                      className="bg-white border border-[#DCCCB4] rounded-xl p-6"
                    >
          ...
          <section
            id="teaser-deck"
            className="py-24 bg-[#37424B] text-white px-4 sm:px-6 lg:px-8 border-y border-[#C5A059]/30 relative overflow-hidden"
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {TEASER_LISTINGS.map(
                  (lot) => (
                    <div
                      key={lot.id}
                      className="bg-[#6D6760]/40 border border-white/10 rounded-xl p-5 shadow-xl flex flex-col justify-between min-h-[320px] relative overflow-hidden group"
                    >
                      <div className="space-y-4">
                        <div className="border-b border-white/10 pb-2 flex items-center justify-between">
                          ...
                          <span className="text-[9px] font-mono bg-white/10 px-2 py-0.5 rounded text-slate-300">
                            {lot.id}
                          </span>
                        </div>
                        ...
                        <div className="bg-black/40 rounded-lg p-3 border border-white/5 text-[10px] font-mono space-y-1 relative">
                          <div className="filter blur-xs select-none space-y-1 opacity-40">
                            <div>
                              COMMERCIAL RATE: ₹X,XXX / MT
                            </div>
                            <div>
                              CRUSHER: [Encrypted Plant]
                            </div>
                            <div>
                              DISPATCH: [Restricted Gate]
                            </div>
                          </div>
                  ...
                  <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-6 text-left flex-1">
                    <div className="flex items-center justify-between border-b pb-3.5">
                      ...
                      <button
                        ...
                        className="p-1 rounded-md bg-slate-100 text-slate-500 hover:text-slate-800"
                      >
                        <FiX size={18} />
                      </button>
                    </div>
                    ...
                    <div className="bg-[#F4F2EE] border border-[#DCCCB4] p-3.5 rounded-xl space-y-2 text-xs">
                      <span className="text-[9px] font-mono bg-[#37424B] text-[#C5A059] px-2 py-0.5 rounded font-bold">
                        {activeDrawerLot.id}
                      </span>

                      <div className="text-sm font-bold text-[#37424B]">
                        {activeDrawerLot.region}
                      </div>

                      <div className="text-xs text-[#6D6760] font-mono">
                        Division:{' '}
                        {activeDrawerLot.division}
                      </div>
                      ...
                      <div className="text-xs text-[#6D6760] font-mono">
                        Base Price: INR{' '}
                        {activeDrawerLot.price}/MT
                      </div>
                    </div>

                    <div className="space-y-2">
                      <label className="block text-[10px] font-mono uppercase text-[#6D6760]">
                      ...
                      <input
                        ...
                        className="w-full border border-[#DCCCB4] rounded p-3 font-mono text-xs text-[#37424B]"
                      />
