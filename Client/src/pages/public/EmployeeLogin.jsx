  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FBF7EF] py-16 px-4 relative overflow-hidden font-sans antialiased">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-24 -right-24 w-[500px] h-[500px] bg-[#C99B38]/5 rounded-full filter blur-3xl" />
        <div className="absolute -bottom-24 -left-24 w-[500px] h-[500px] bg-[#0B2D5B]/5 rounded-full filter blur-3xl" />
      </div>
      ...
      >
        ...
        <div className="text-center space-y-2 mb-8">
          <div className="flex justify-center mb-3">
            <div className="h-10 w-10 bg-[#0B2D5B] rounded-sm border border-[#C99B38]/20 flex items-center justify-center shadow-sm">
              <FiShield className="h-4 w-4 text-[#C99B38]" />
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.button
            ...
            whileHover={{ y: -0.5 }}
            whileTap={{ y: 0 }}
            className="w-full mt-4 bg-[#0B2D5B] text-[#FBF7EF] text-xs font-medium tracking-wider py-3 rounded border border-transparent hover:border-[#C99B38]/30 transition-all shadow-sm uppercase"
          >
            {loading ? 'Establishing Link...' : 'Sign In'}
          </motion.button>
        </form>

        <div className="text-center text-[11px] text-[#0B2D5B]/70 border-t border-[#0B2D5B]/5 mt-6 pt-4">
          <p>
            Need a client account?{' '}
            <Link to="/login" className="font-semibold text-[#C99B38] hover:underline">
              Client login
            </Link>
            {' / '}
