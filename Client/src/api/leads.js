...
export const leadsApi = {
  ...
  async deleteLead(leadId) {
    try {
      ...
      /*
       * Latest backend exposes DELETE /leads/:id.
       * Keep the legacy admin endpoint only as a route-compatibility fallback.
       * Do not retry authorization/validation/server failures through a second
       * endpoint because that can hide the real failure.
       */
      if (
        status !== 404 &&
        status !== 405
...
export const softLeadsApi = {
  ...
  async updateSoftLeadDetails(
    leadId,
    details
  ) {
    const response =
      await axiosInstance.patch(
        `/soft-leads/${leadId}/details`,
        details
      );
  ...
  async retryCrmSync(leadId) {
    const response =
      await axiosInstance.post(
        `/soft-leads/${leadId}/retry-crm`
      );
