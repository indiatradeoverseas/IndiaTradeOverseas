...
async function approveQuotation({
  ...
  try {
    ...
    if (quotation.commercialTerms) {
      ...
      if(commercial.validateTerms(quotation.commercialTerms).length || !quotation.commercialTerms.confirmedAt) throw Object.assign(new Error('Operations terms are incomplete or expired.'),{code:'QUOTATION_TERMS_INVALID'});
      if(Math.abs(finalPrice-commercial.total(quotation.commercialTerms))>0.005) throw Object.assign(new Error('Approved amount must match the Operations-confirmed total. Request a revision for changed terms.'),{code:'QUOTATION_TERMS_INVALID'});
    }
    quotation.status =
      'APPROVED';
