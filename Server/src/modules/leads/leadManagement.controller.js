...
async function auditEvidenceSuccess(req, {
  actionType,
  entityType = 'LEAD',
  entityId,
  leadId = null,
  ...
  const safeEntityId = cleanText(
    entityId || recordingId || leadId || 'LEAD_EVIDENCE',
    200
  );

  await safeEvidenceAudit({
    actorId: getEvidenceAuditActorId(req.user),
    actionType,
    entityType,
    entityId: safeEntityId,
    severity: 'LOW',
    ipAddress: getEvidenceRequestIp(req),
    deviceHash: cleanText(req.headers?.['x-device-hash'], 256),
    metadata: {
      evidenceType: cleanText(evidenceType, 80).toUpperCase(),
      operation: cleanText(operation, 80).toUpperCase(),
      leadId: leadId ? cleanText(leadId, 100) : '',
      recordingId: recordingId ? cleanText(recordingId, 100) : '',
      ...metadata,
    },
  });
}

function normalizeInternalSource(value, fallback = 'MANUAL') {
...
function normalizePriority(value) {
  const priority = cleanText(value, 40).toUpperCase();
...
function isManagementUser(user) {
  const role = cleanText(user?.role, 80).toUpperCase();
  const department = cleanText(user?.department, 80).toUpperCase();
  const position = cleanText(user?.position, 120).toUpperCase();
...
function getContactOutcomeFromActivity(actionType, explicitOutcome) {
  const explicit = cleanText(explicitOutcome, 40)
    .toUpperCase()
    .replace(/[\s-]+/g, '_');
  ...
  const normalizedAction = cleanText(actionType, 80)
    .toUpperCase()
    .replace(/[\s-]+/g, '_');
...
async function advanceLeadForCommunication({
  ...
  if (!lead?._id || outcome === 'NONE') {
    return lead;
  }

  let currentLead = lead;
  let currentStatus = normalizeCrmStatus(
    currentLead.crmStatus
  ) || CRM_STATUS.NEW;

  if (
    currentStatus === CRM_STATUS.WON ||
    currentStatus === CRM_STATUS.LOST
  ) {
    return currentLead;
  }
  ...
  if (
    outcome === 'ATTEMPTED' &&
    currentStatus === CRM_STATUS.NEW
  ) {
    const result = await transitionLeadCrmStatus({
      leadId: currentLead._id,
      toStatus: CRM_STATUS.CONTACT_ATTEMPTED,
      actorId,
      note: note || 'Buyer contact attempt logged.',
      nextFollowupAt,
    });
  ...
  if (outcome === 'CONTACTED') {
    if (currentStatus === CRM_STATUS.NEW) {
      const attempted = await transitionLeadCrmStatus({
        leadId: currentLead._id,
        toStatus: CRM_STATUS.CONTACT_ATTEMPTED,
        actorId,
        note: 'Contact attempt recorded before confirmed buyer contact.',
        nextFollowupAt,
      });

      currentLead = attempted.lead;
      currentStatus = CRM_STATUS.CONTACT_ATTEMPTED;
    }

    if (currentStatus === CRM_STATUS.CONTACT_ATTEMPTED) {
      const contacted = await transitionLeadCrmStatus({
        leadId: currentLead._id,
        toStatus: CRM_STATUS.CONTACTED,
        actorId,
        note: note || 'Buyer contact confirmed.',
        nextFollowupAt,
      });
    ...
    if (
      ...
      if (!currentLead.firstResponseAt) {
      ...
      currentLead = await Lead.findByIdAndUpdate(
        currentLead._id,
        {
          $set: set,
        },
      ...
      await queueCrmResync(currentLead._id);
...
async function createInternalLeadOpportunity({
  ...
  const productCategory = cleanText(
    payload.productCategory || payload.material,
    100
  );
  ...
  if (!payload.phone && !payload.mobileNumber) {
  ...
  if (!productCategory) {
    const error = new Error('productCategory is required.');
    error.code = 'VALIDATION_FAILED';
    throw error;
  }
  ...
  const timeline = cleanText(
    payload.timeline || payload.purchaseTimeline,
    120
  );

  const targetDate = parseOptionalDate(
    payload.targetDate,
    'targetDate'
  );
  ...
  const quantityValue = payload.quantityValue !== undefined
    ? Number(payload.quantityValue)
    : parseQuantityValue(quantity);

  const safeSource = normalizeInternalSource(
  ...
  const contactResolution = await resolveOrCreateContact({
    ...
    gst,
    gstVerified: payload.gstVerified === true,
    consent: payload.consent || {},
    actorId: actorUser?._id || null,
  });

  const scoring = scoreAndClassifyLead({
    truckCount:
      payload.truckCount ??
      payload.trucks ??
    ...
    isImmediateRequirement:
      payload.isImmediateRequirement === true,

    withinSevenDays:
      payload.withinSevenDays === true ||
      payload.within7Days === true,

    isPriorityAServiceableMarket:
      payload.isPriorityAServiceableMarket === true ||
      payload.priorityAServiceableMarket === true,
    ...
    businessVerificationProvided:
      payload.businessVerificationProvided === true ||
      payload.businessVerified === true,

    completedPriceCheck:
      payload.completedPriceCheck === true ||
      payload.priceCheckCompleted === true,

    returnVisit:
      payload.returnVisit === true ||
      payload.isReturnVisit === true,
  ...
  const leadCode = `LD-${Date.now()}-${crypto
    .randomUUID()
    .slice(0, 8)}`;
  ...
  const lead = await Lead.create({
    ...
    contactId:
      contactResolution.contact?._id || null,
    ...
    productCategory,
    product: cleanText(payload.product, 150),
    productVariant: cleanText(payload.productVariant, 150),
    grade: cleanText(payload.grade, 150),
    specification: cleanText(payload.specification, 1000),
    ...
    quantityValue:
      Number.isFinite(quantityValue)
        ...
        : 0,
    quantityUnit:
      cleanText(payload.quantityUnit, 30) || 'MT',
    quantityBand:
      cleanText(payload.quantityBand, 100),

    destination:
      cleanText(payload.destination || payload.location, 200),
    timeline,
    targetDate,

    leadValue:
      Number(payload.leadValue || 0) || 0,

    score: scoring.score,
    priority: scoring.priority,

    crmStatus: CRM_STATUS.NEW,
    crmStatusChangedAt: new Date(),
    crmStatusChangedBy:
      actorUser?._id || null,
    ...
    consent: {
      ...
      marketingAllowed:
        payload.consent?.marketingAllowed === true,
      analyticsAllowed:
        payload.consent?.analyticsAllowed === true,
      advertisingAllowed:
        payload.consent?.advertisingAllowed === true,
      privacyVersion:
        cleanText(
      ...
      trackingConsentCapturedAt:
        parseOptionalDate(
          payload.consent?.trackingConsentCapturedAt,
          'trackingConsentCapturedAt'
        ),
      capturedAt:
        payload.consent
    ...
    attribution: {
      utmSource:
        cleanText(payload.attribution?.utmSource, 150),
      utmMedium:
        cleanText(payload.attribution?.utmMedium, 150),
      utmCampaign:
        cleanText(payload.attribution?.utmCampaign, 200),
      utmContent:
        cleanText(payload.attribution?.utmContent, 200),
      utmTerm:
        cleanText(payload.attribution?.utmTerm, 200),
      gclid:
        cleanText(payload.attribution?.gclid, 250),
      fbclid:
        cleanText(payload.attribution?.fbclid, 250),
      campaignId:
        cleanText(payload.attribution?.campaignId, 150),
      adSetId:
        cleanText(payload.attribution?.adSetId, 150),
      adId:
        cleanText(payload.attribution?.adId, 150),
      creativeId:
        cleanText(payload.attribution?.creativeId, 150),
      creative:
        cleanText(payload.attribution?.creative, 200),
      landingPage:
        cleanText(payload.attribution?.landingPage, 250),
      landingPageType:
        cleanText(payload.attribution?.landingPageType, 100),
      analyticsSessionId:
        cleanText(payload.attribution?.analyticsSessionId, 160),
    },
    ...
    originalPayload: {
      ...
      contactResolutionReason:
        cleanText(contactResolution.reason, 200),
      contactConflictIds:
        Array.isArray(contactResolution.conflictContactIds)
  ...
  await safeAudit({
    ...
    entityId: lead._id,
    severity:
      contactResolution.status === 'AMBIGUOUS'
        ? 'MEDIUM'
        : 'LOW',
    metadata: {
...
async function createManualLead(req, res, next) {
  try {
    const lead = await createInternalLeadOpportunity({
      payload: req.body,
      actorUser: req.user,
      source: normalizeInternalSource(
        req.body.source,
        'MANUAL'
      ),
    });
    ...
    if (
      error?.code === 'VALIDATION_FAILED' ||
      error?.code === 'CONTACT_PHONE_INVALID' ||
      error?.code === 'CONTACT_EMAIL_INVALID' ||
      error?.code === 'CONTACT_RESOLUTION_KEY_REQUIRED'
    ) {
      return fail(
        res,
        400,
...
async function uploadVoiceNote(req, res, next) {
  try {
    ...
    const allowed = await hasLeadEvidenceAccess({
      user: req.user,
      lead,
    });
    ...
    if (!Array.isArray(lead.voiceNotes)) {
      lead.voiceNotes = [];
    }
    ...
    await auditEvidenceSuccess(req, {
      actionType: 'VOICE_NOTE_UPLOADED',
      entityType: 'LEAD',
      ...
      operation: 'UPLOAD',
      metadata: {
        noteIndex: lead.voiceNotes.length - 1,
      },
    });
...
async function streamVoiceNote(req, res, next) {
  try {
    ...
    const voiceNote = lead.voiceNotes?.[noteIndex];
    ...
    if (!filePath || !fs.existsSync(filePath)) {
      try {
        ...
        await auditEvidenceSuccess(req, {
          ...
          metadata: {
            noteIndex,
            storageSource: 'PRODUCTION_PROXY',
          },
        });
...
async function addActivity(req, res, next) {
  try {
    ...
    lead = await advanceLeadForCommunication({
      lead,
    ...
    if (
      nextFollowupAt &&
      String(lead.nextFollowupAt || '') !==
        String(nextFollowupAt)
    ) {
      lead.nextFollowupAt = nextFollowupAt;
      await lead.save();
      await queueCrmResync(lead._id);
    }
...
async function logEmailActivity(req, res, next) {
  try {
    ...
    const decryptedEmail = lead.emailEncrypted
      ? decryptText(lead.emailEncrypted)
      : '';
    ...
    if (
      decryptedEmail &&
      decryptedEmail !== 'DECRYPTION_ERROR'
    ) {
      try {
        const {
          sendEmail,
        ...
        const safeHtmlBody = escapeHtml(body)
          .replace(/\n/g, '<br/>');
  ...
  try {
    ...
    if (leadId) {
      ...
      if (material) {
        lead.productCategory =
          cleanText(material, 100);
      }

      if (quantity) {
        lead.quantity =
          cleanText(quantity, 120);
      }

      if (location) {
        lead.destination =
          cleanText(location, 200);
      }
      ...
      lead = await createInternalLeadOpportunity({
        payload: {
          customerName:
            customerName ||
            'Direct Customer',
    ...
    const callRecording =
      await CallRecording.create({
        ...
        /*
         * Avoid duplicating a decrypted Lead phone into the recording.
         * If the UI supplied an unlinked phone, preserve only the masked form.
         */
        mobileNumber:
          lead?.phoneMasked ||
          (
        ...
        material:
          cleanText(material, 150),
        quantity:
          cleanText(quantity, 120),
        location:
          cleanText(location, 200),
        serialNo:
          cleanText(serialNo, 100),
        audioPath:
          getRelativePath(
            req.file.path
          ),
        ...
        size:
          req.file.size || 0,
        duration:
          cleanText(duration, 100),
        notes:
          cleanText(notes, 4000),
        leadPriority:
          recordingPriority,
      });
    ...
    await auditEvidenceSuccess(req, {
      actionType: 'CALL_RECORDING_UPLOADED',
      entityType: 'CALL_RECORDING',
      entityId: String(callRecording._id),
      leadId: lead?._id ? String(lead._id) : null,
      recordingId: String(callRecording._id),
      evidenceType: 'CALL_RECORDING',
      operation: 'UPLOAD',
      metadata: {
        linkedToLead: Boolean(lead?._id),
        priority: callRecording.leadPriority || '',
        driveCopyAvailable: Boolean(callRecording.driveFileId),
      },
    });
...
async function getCallRecordings(req, res, next) {
  try {
    ...
    const isManagerOrAdmin =
      isBroadEvidenceReviewer(req.user);
    ...
    if (priority) {
      const recordingPriorityFilter =
        toRecordingPriority(priority);

      if (!recordingPriorityFilter) {
        return fail(
          res,
          400,
          'INVALID_PRIORITY',
    ...
    const rawRecordings = await CallRecording.find(filter)
      .populate('leadId', 'customerName leadCode companyName priority stage crmStatus assignedTo assignedDepartment')
      .sort({ createdAt: -1 })
      .lean();
...
async function streamCallRecording(req, res, next) {
  try {
    ...
    if (
      !recordingId ||
      !mongoose.isValidObjectId(recordingId)
    ) {
      return fail(
        res,
        404,
    ...
    const recording = await CallRecording.findById(
      recordingId
    );
    ...
    const lead = recording.leadId
      ? await Lead.findById(recording.leadId)
      : null;

    const allowed = await hasCallRecordingAccess({
      user: req.user,
      recording,
      lead,
    });
    ...
    if (!filePath || !fs.existsSync(filePath)) {
      if (recording.driveFileId) {
        try {
          ...
          const driveStream = await getDriveFileStream(
            recording.driveFileId
          );

          if (driveStream) {
            await auditEvidenceSuccess(req, {
              actionType: 'CALL_RECORDING_VIEWED',
              entityType: 'CALL_RECORDING',
...
async function updateCallRecordingRemark(req, res, next) {
  try {
    ...
    if (!canManageCallRecordingRemark(req.user)) {
      await auditEvidenceAccessDenied(
        req,
        {
          actionType:
    ...
    await auditEvidenceSuccess(req, {
      actionType: 'CALL_RECORDING_UPDATED',
      entityType: 'CALL_RECORDING',
      entityId: String(recording._id),
      leadId: recording.leadId ? String(recording.leadId) : null,
      recordingId: String(recording._id),
      evidenceType: 'CALL_RECORDING',
      operation: 'UPDATE_MANAGER_REMARK',
    });
...
async function uploadLOIDocument(req, res, next) {
  try {
    ...
    /*
     * IMPORTANT:
     * Uploading a document is evidence, not a CRM lifecycle transition.
     * The previous controller jumped early-stage Leads directly to
     * LOI_PO_PENDING. That bypassed CONTACTED / QUALIFIED / QUOTE SENT /
     * NEGOTIATION. Stage movement now remains explicit and validated.
     */
    await lead.save();
    ...
    await auditEvidenceSuccess(req, {
      actionType: 'LOI_DOCUMENT_UPLOADED',
      entityType: 'LEAD',
      entityId: String(lead._id),
      leadId: String(lead._id),
      evidenceType: 'LOI_DOCUMENT',
      operation: 'UPLOAD',
      metadata: {
        documentIndex: lead.loiDocuments.length - 1,
        crmStatus: lead.crmStatus,
        driveCopyAvailable: Boolean(loiObj.driveFileId),
      },
    });
...
async function streamLOIDocument(req, res, next) {
  try {
    ...
    const loiDoc =
      lead.loiDocuments?.[documentIndex];
    ...
    if (
      ...
      await auditEvidenceSuccess(req, {
        actionType: 'LOI_DOCUMENT_VIEWED',
        entityType: 'LEAD',
        entityId: String(lead._id),
        leadId: String(lead._id),
        evidenceType: 'LOI_DOCUMENT',
    ...
    /*
     * Do not redirect to a persistent Drive web-view URL. The authenticated
     * backend stream remains the authorization boundary for commercial
     * evidence.
     */
    return fail(
      res,
...
async function updateCallRecordingStatus(req, res, next) {
  try {
    ...
    /*
     * Completing a recording review does NOT mean:
     * - buyer contacted;
     * - requirement captured;
     * - Lead qualified.
     *
     * Those business facts must be recorded through explicit CRM/activity
     * actions.
     */
