...
function scoreCandidate({
  ...
  // Product/department match is the strongest routing constraint.
  score +=
    100;
...
async function findBestOwner({
  ...
  // Compatibility fallback for installations where salespeople are stored in
  // the generic SALES department rather than product-specific departments.
  if (
    !candidates.length
  ) {
    candidates =
...
async function createAssignmentNotification({
  ...
  try {
    ...
    // Never rethrow. Ownership/persistence must not be undone by a
...
async function autoRouteLead(
  ...
  /*
   * Never silently replace a legitimate existing owner. Manual reassignment
   * remains an explicit CRM action through lead.service.js.
   */
  if (
    !force &&
    hasOwner(
  ...
  /*
   * A previously established department is useful for legacy/recovery rows
   * where product text is not strong enough to resolve a routing rule.
   */
  ...
  /*
   * Assignment must never regress a Lead that already moved past the initial
   * operational stage. Only a genuine NEW_LEAD becomes ASSIGNED.
   */
  const currentStage =
    cleanText(
      lead.stage ||
  ...
  const set = {
    ...
    // Preserve the canonical lifecycle status. Assignment != qualification.
    crmStatus:
      lead.crmStatus ||
      'NEW',
  ...
  // Keep the caller's in-memory document useful for subsequent automation.
  if (
    typeof lead.set ===
    'function'
  ) {
