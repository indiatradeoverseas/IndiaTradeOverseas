...
function evaluateStoneEligibility() {
  return { status: 'REVIEW_REQUIRED', reason: 'OPERATIONS_SERVICEABILITY_CONFIRMATION_REQUIRED' };
}
...
async function createWebsiteLeadRecord(payload = {}) {
  ...
  const {
    ...
    leadValue: 0,
    targetDate, timeline, companyName, gst, completedPriceCheck: !quickCapture
  });
  ...
  const requirementDetails = {
    ...
    packaging: cleanText(
      details.packaging || payload.packaging,
      500
    ),
    tradeType: cleanText(
      details.tradeType || payload.tradeType,
      20
    ).toUpperCase(),
    incoterm: cleanText(
      details.incoterm || payload.incoterm,
      100
    ),
    ...
    objective: cleanText(
      details.objective || payload.objective,
      250
    ),
    ...
    if (error?.code === 11000) {
      ...
      if (alreadyCreated) {
        await logDatabaseWrite(
          ...
          'SUCCESS',
          {
            leadId: alreadyCreated._id,
            entityType: 'LEAD',
            entityId:
              alreadyCreated.leadCode ||
              String(alreadyCreated._id),
            provider: 'MONGODB',
            idempotencyKey: submissionId,
            metadata: {
              persisted: true,
...
async function processWebsiteLeadAutomation(leadId) {
  const lead = await Lead.findOneAndUpdate(
...

    {
      $set: {
        ...
        automationLastAttemptAt: new Date()
      },
  ...
  await logRetry(
      ...
      : 'PENDING',
    {
      ...
      retryCount: Math.max(0, Number(lead.automationAttempts) - 1),
      metadata: {
        automationAttempt: lead.automationAttempts,
        source: 'WEBSITE'
      }
    }
  );
  ...
  try {
    ...
    await logRetry(
      'WEBSITE_LEAD_AUTOMATION',
      Number(lead.automationAttempts) >= 5
        ? 'MANUAL_RECOVERY'
        : 'RETRY_SCHEDULED',
      {
        leadId: lead._id,
        ...
        entityId: lead.leadCode || String(lead._id),
        provider: 'INTERNAL_WORKER',
        retryCount: Number(lead.automationAttempts) || 0,
        error,
        metadata: {
          automationAttempt: lead.automationAttempts,
          finalStatus: 'FAILED'
...
async function updateWebsiteLeadProgressiveProfile(input = {}) {
  ...
  if (
    ...
    error.code = 'WEBSITE_LEAD_NOT_FOUND';
    throw error;
  }
  ...
  if (resolvedContact?._id) {
    ...
    set['contactResolution.status'] =
      contactResolution.status ||
      'MATCHED';

    set['contactResolution.method'] =
      contactResolution.method ||
      'PHONE';

    set['contactResolution.resolvedAt'] =
      new Date();
  }
...
async function retryPendingWebsiteLeadAutomations(
  ...
  if (staleRecoveryResult.modifiedCount > 0) {
    await logRetry(
      'WEBSITE_LEAD_AUTOMATION_STALE_RECOVERY',
      'RETRY_SCHEDULED',
      {
        entityType: 'WORKER',
        entityId: 'WEBSITE_LEAD_AUTOMATION',
        provider: 'INTERNAL_WORKER',
        retryCount: 0,
        metadata: {
          recoveredCount: staleRecoveryResult.modifiedCount
        }
      }
    );
  }
