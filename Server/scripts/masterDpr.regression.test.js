...
test('Phase 9 reorder flow is explicitly classified and order data stays linked to a Lead',()=>{
  const path=require('node:path');
  const portal=fs.readFileSync(path.join(__dirname,'../src/modules/orders/customerPortal.routes.js'),'utf8');
  const Order=require('../src/modules/orders/customerOrder.model');
  assert.match(portal,/captureMode\s*:\s*['"]REPEAT_ORDER['"]/);
  assert.ok(Order.schema.path('leadId'));
  assert.ok(Order.schema.path('quoteId'));
  assert.ok(Order.schema.path('customerAccountId'));
  assert.ok(Order.schema.path('documents'));
});
